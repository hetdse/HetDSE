...
from config import case_names

#warnings.filterwarnings('ignore', category=ConvergenceWarning)

base_estimator_name = "PolyLinear"
...
def get_base_estimator(base_estimator_name = "GBRT"):
    ...
    match base_estimator_name:
        ...
        case "BagGBRT":
            SPACE = [
                skopt.space.Integer(2, 40, name='n_estimators', prior='uniform'),
...
class ParamsEvaluate():
    ...
    def select_model(self, model, base_estimator_name, hbo_param):
        self.base_estimator_name = base_estimator_name
        #self.param_space = param_space
        self.hbo_param = hbo_param
        self.model = model

    def find_best_model(self, param_space):
        @skopt.utils.use_named_args(dimensions=param_space)
        def fitness_wrapper(**kwargs):
            print(f"kwargs={kwargs}")
            match self.base_estimator_name:
                case "PolyLinear":
                    model = Pipeline([
                        ("poly", PolynomialFeatures(degree=kwargs['poly__degree'])),
                        ("std_scaler", StandardScaler()),
                        ("lin_reg", LinearRegression())
                    ])
                case "_":
                    print(f"no match base_estimator_name={self.base_estimator_name}")
                    model = None
                    exit(1)
            return self.fitness(model, kwargs)
        search_result = skopt.gp_minimize(func=fitness_wrapper, dimensions=param_space, **self.hbo_param)
        return search_result


    def fitness(self, model, kwargs):
        self.n += 1
        if True:
            ...
            print("Iteration %d score= %0.5f (+/- %0.5f ) %s" % (self.n, scores.mean(), scores.std() * 2, kwargs))
...
for case_name in [case_name]:
    ...
    base_estimator_name_log_case = case_name + "-" + base_estimator_name_log
    print(base_estimator_name_log_case)
    result_log = open("log_tune_model/" + base_estimator_name_log_case + ".log", "w")

    evaluator.select_model(model=base_estimator, base_estimator_name=base_estimator_name, hbo_param=HPO_PARAMS)
    search_result = evaluator.find_best_model(param_space=SPACE, )

    param_dict = to_named_params(search_result.x, SPACE)
    print("----------------------------------------------------------------------------------------------------\n")
    print(f"best func_value= {search_result.fun} param_dict={param_dict}")

    hostname = socket.getfqdn(socket.gethostname())

    result_log.write(f"search_result={search_result}\n")
    result_log.write(
        f"best func_value= {search_result.fun} (+/- {scores_return_min.std() * 2} ) iter= {scores_return_min_index} param_dict= {param_dict} hostname= {hostname}\n")
    result_log.close()
    ...
    result_summary_log.write(
        f"%-10f (+/- %-10f ) %-35s %-15s %s \n"
        % (search_result.fun, scores_return_min.std() * 2, base_estimator_name_log_case, hostname, param_dict))
