...
def create_result_multiobj(Xi, yi, yi2,
                           best, func_vals,
                           predict_errors, predict_error_unlabelled_mapes, Rsquares,
                           non_uniformitys, coverage,
                           result_array_sample_map,
                           space=None, rng=None, specs=None, models=None):
    """
    Initialize an `OptimizeResult` object.
    ...
    #best = np.argmin(yi)
    res.x = best
    res.func_vals = np.asarray(func_vals)
    res.fun = res.func_vals[-1] if len(func_vals) else -1
    res.predict_errors = np.asarray(predict_errors)
    res.predict_error = res.predict_errors[-1] if len(predict_errors) else [-1, -1]
    res.predict_error_unlabelled_mapes = np.asarray(predict_error_unlabelled_mapes)
    res.predict_error_unlabelled_mape = res.predict_error_unlabelled_mapes[-1] if len(predict_error_unlabelled_mapes) else -1
    res.Rsquares = np.asarray(Rsquares)
    res.Rsquare = res.Rsquares[-1] if len(Rsquares) else -1
    res.non_uniformitys = np.asarray(non_uniformitys)
    res.non_uniformity = res.non_uniformitys[-1] if len(non_uniformitys) else -1
    res.coverages = np.asarray(coverage)
    res.coverage = res.coverages[-1] if len(coverage) else -1
    res.used_map = {}
    res.result_array_sample_map = result_array_sample_map
